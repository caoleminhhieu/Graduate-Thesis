...
class Evaluation_metrics():
    ...
    def cal_total_metrics(self, pred, mask):
        ...
        if y == 0:
            ...
            mask[mask >= 0.5] = 1
            mask[mask < 0.5] = 0
            Q = alpha * self._S_object(pred, mask) + \
                (1 - alpha) * self._S_region(pred, mask)
            if Q.item() < 0:
    ...
    def _eval_pr(self, y_pred, y, num):
        if self.device:
            prec, recall = torch.zeros(num).to(
                self.device), torch.zeros(num).to(self.device)
    ...
    def _centroid(self, mask):
        ...
        if mask.sum() == 0:
            ...
            total = mask.sum()
            if self.device:
                i = torch.from_numpy(np.arange(0, cols)).to(
                    self.device).float()
                j = torch.from_numpy(np.arange(0, rows)).to(
                    self.device).float()
            else:
